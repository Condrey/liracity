		...
		<div className="flex size-full min-h-dvh flex-col justify-center p-4">
			<Card className="mx-auto w-full max-w-md">
				<CardHeader>
					...
					<CardDescription>
						Please check your email for the link. If you did not receive it, please tap on the resend verification link
						button below
					</CardDescription>
				</CardHeader>
				<CardContent>
					<Form {...form}>
						<form onSubmit={form.handleSubmit(handleEmailResend)} className="space-y-4">
							{error && (
				...
				</CardContent>
				{
					<CardFooter className={cn("text-xs text-muted-foreground", isRefetching ? "visible" : "invisible")}>
						<LoaderIcon className="mr-2 size-4 animate-spin" /> checking for token verification
					</CardFooter>
				}
			</Card>
		</div>
