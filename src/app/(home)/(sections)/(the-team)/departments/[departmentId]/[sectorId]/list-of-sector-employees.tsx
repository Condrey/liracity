...
export default function ListOfSectorEmployees({ sector }: ListOfSectorEmployeesProps) {
	const employees = sector.employees;

	return (
		<div>
			{!employees.length ? (
				<EmptyContainer message={`There are no staffs added to ${sector.name} sector yet.`}>
					<ButtonAddEditEmployee departmentalSectorId={sector.id} variant={"secondary"}>
						Create new one
				...
				<DataTable
					data={employees}
					columns={useSectorEmployeeColumns}
