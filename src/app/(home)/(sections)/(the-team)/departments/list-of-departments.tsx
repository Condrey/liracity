...
export default function ListOfDepartments({ departments }: ListOfDepartmentsProps) {
	...
	return (
		<div>
			{status === "error" ? (
				<ErrorContainer errorMessage="An error occurred while fetching departments. Please try again!" query={query} />
			) : status === "success" && !data.length ? (
				<EmptyContainer message={"There are no departments in the database yet. Please add"}>
					<ButtonAddEditDepartment variant={"secondary"}>Add department</ButtonAddEditDepartment>
				</EmptyContainer>
			) : (
				<div className="space-y-4">
					...
					>
						{isAuthorized && (
							<ButtonAddEditDepartment size="sm" variant={"default"} className='sm:after:content-["New"]'>
								<PlusIcon />
