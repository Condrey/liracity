...
export const useDepartmentsColumns: ColumnDef<DepartmentData>[] = [
	...
	},
	{
		...
		header({ column }) {
			return <DataTableColumnHeader className="hidden md:flex" column={column} title="Head of Department" />;
		},
		cell({ row }) {
	...
	},
	{
		...
		header({ column }) {
			return <DataTableColumnHeader className="hidden sm:flex" column={column} title="Department Sectors" />;
		},
		cell({ row }) {
			...
			return (
				<div className="hidden flex-col sm:flex">
					{numberOfDepartmentalSectors === 0 ? (
						<>
							{isAuthorized ? (
								<ButtonAddEditDepartmentalSector variant={"outline"} departMentId={row.original.id}>
									<PlusIcon /> New sector
						...
						<div className="flex gap-1">
							<Badge variant={"secondary"} className="flex w-fit flex-wrap">
								{departmentalSectors[0].name}
							</Badge>{" "}
							<span className="text-xs text-muted-foreground italic">only</span>
						</div>
					) : (
						<div className="inline-flex max-w-xs flex-wrap gap-1">
							<div>
								{formatNumber(numberOfDepartmentalSectors)} sectors{" "}
								<span className="text-xs text-muted-foreground italic">including</span>
							</div>
							<div className="inline-flex flex-wrap gap-1 text-xs text-muted-foreground">
								{departmentalSectors.slice(-3).map((sector) => (
									<Badge key={sector.id} variant={"secondary"}>
										<span className="text-wrap break-all hyphens-auto">{sector.name}</span>
									</Badge>
								))}
							</div>
						</div>
