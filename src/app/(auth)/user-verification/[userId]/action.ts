...
export async function verifyUser(input: VerifyUserSchema): Promise<{ error: string }> {
	console.log("verifying user starts");
	const { email, id, name, telephone, username, password } = verifyUserSchema.parse(input);
	try {
