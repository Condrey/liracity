	...
	return (
		<>
			<Button
				...
				{...props}
			/>
			<DeleteDepartmentalSectorDialog open={open} setOpen={setOpen} departmentalSector={departmentalSector} />
		</>
	);
}
				...
				<DialogHeader>
					<DialogTitle className="text-destructive">
						<AlertTriangleIcon
							...
							strokeWidth={0.8}
						/>
						<span className="uppercase">Delete {departmentalSector.name} sector</span>
					</DialogTitle>
					<DialogDescription>Dangerous! Please note that this action is irreversible</DialogDescription>
				</DialogHeader>
				<p>
					This will delete <strong>{departmentalSector.name}</strong> sector from the{" "}
					<strong>{departmentalSector.departMent?.name}</strong> department and all its <strong>officers</strong> from
