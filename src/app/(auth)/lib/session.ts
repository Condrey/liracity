...
export async function createSession(token: string, userId: string): Promise<LuciaSessionWithToken> {
	const now = new Date();
...
export async function getSessionById(sessionId: string): Promise<SessionWithUserData | null> {
	const now = new Date();
