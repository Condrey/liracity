		...
		>
			<Link href={getNavigationLinkWithPathnameWithoutUpdate(`${LINK_EVENTS}/${item.slug}`)}>
				...
				<ItemHeader className="flex-col items-start gap-0.5">
					<ItemTitle className="mb-1 line-clamp-2 font-semibold uppercase transition-all group-hover/item:font-bold">
						{item.title}
					</ItemTitle>
					...
					<ItemDescription>
						<TipTapViewer content={item.summary || item.description} className="text-start text-sm md:text-sm" />
					</ItemDescription>
				</ItemHeader>
			</Link>
		</Item>
