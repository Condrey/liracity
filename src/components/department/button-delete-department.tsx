...
export default function ButtonDeleteDepartment({ department, variant, ...props }: ButtonDeleteDepartmentProps) {
	const [open, setOpen] = useState(false);

	return (
		<>
			<Button onClick={() => setOpen(true)} variant={variant || "destructive"} {...props} />
			<DeleteDepartmentDialog open={open} setOpen={setOpen} department={department} />
		</>
	);
}
...
export function DeleteDepartmentDialog({ department, open, setOpen }: DeleteDepartmentDialogProps) {
				...
				<DialogHeader>
					<DialogTitle className="text-destructive">
						<AlertTriangleIcon
							...
							strokeWidth={0.8}
						/>
						<span className="uppercase">Delete {department.name} department</span>
					</DialogTitle>
					<DialogDescription>Dangerous! Please note that this action is irreversible</DialogDescription>
				</DialogHeader>
				<p>
					This will delete <strong>{department.name}</strong> department and all its <strong>sectors</strong> from the
					database. Continue with caution.
				</p>
				<DialogFooter>
