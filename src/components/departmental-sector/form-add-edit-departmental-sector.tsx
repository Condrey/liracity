...
export default function FormAddEditDepartmentalSector({
	...
	const form = useForm<DepartmentalSectorSchema>({
		...
		defaultValues: {
			...
			departMentId: departmentalSectorToEdit?.departMentId || departMentId || ""
		}
	});
	...
	return (
		<ResponsiveDrawer
			open={open}
			setOpen={setOpen}
			title={departmentalSectorToEdit ? "Update departmental sector" : "Add departmental sector"}
		>
			<Form {...form}>
				<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<NumberInput placeholder="e.g 1 for highest rank" {...field} />
								</FormControl>
								<FormMessage />
								<FormDescription>Refer to the organography diagram for the hierarchy</FormDescription>
							</FormItem>
						)}
					/>
					<FormField
							...
							<FormItem>
								...
								<FormControl>
									<TipTapEditorWithHeader
										...
										placeholder={`Brief description about ${watchedName || "this"} departmental sector`}
										value={field.value}
										includeHeader={false}
									/>
								</FormControl>
					...
					<div className="flex w-full items-center justify-end gap-4">
						<LoadingButton loading={isPending}>{departmentalSectorToEdit ? "Update" : "Submit"}</LoadingButton>
					</div>
				</form>
			</Form>
		</ResponsiveDrawer>
