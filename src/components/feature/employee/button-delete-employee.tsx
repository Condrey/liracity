...
export default function ButtonDeleteEmployee({ employee, variant, ...props }: ButtonDeleteEmployeeProps) {
	const [open, setOpen] = useState(false);

	return (
		<>
			<Button onClick={() => setOpen(true)} variant={variant || "destructive"} {...props} />
			<DeleteEmployeeDialog open={open} setOpen={setOpen} employee={employee} />
		</>
	);
}
...
export function DeleteEmployeeDialog({ employee, open, setOpen }: DeleteEmployeeDialogProps) {
				...
				<DialogHeader>
					<DialogTitle className="text-destructive">
						<AlertTriangleIcon
							...
							strokeWidth={0.8}
						/>
						<span className="uppercase">Delete {employee.user.name}</span>
					</DialogTitle>
					<DialogDescription>Dangerous! Please note that this action is irreversible</DialogDescription>
				</DialogHeader>
				<p>
					This will delete <strong>{employee.user.name}</strong> employee and all their related data from the database.
