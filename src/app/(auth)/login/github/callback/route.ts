...
import { generateSessionToken, setSessionTokenCookie } from "@/app/(auth)/lib/tokens";
...
export async function GET(request: Request): Promise<Response> {
	...
	if (code === null || state === null || storedState === null) {
		return new Response("Please restart the process", { status: 400, statusText: "Bad Request" });
	}
	if (state !== storedState) {
		return new Response("Please restart the process.", { status: 400, statusText: "Invalid State" });
	}
	...
	try {
		...
		console.error(e);
		return new Response("Please, restart the process.", { status: 400, statusText: "Invalid Code" });
	}
