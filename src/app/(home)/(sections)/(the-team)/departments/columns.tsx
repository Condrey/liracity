...
export const useDepartmentsColumns = (): ColumnDef<DepartmentData>[] => {
  ...
  return [
    ...
    },
    {
      ...
      header({ column }) {
        return (
          <DataTableColumnHeader column={column} title="Head of Department" />
        );
      },
      cell({ row }) {
        const headOfDepartment = row.original.headOfDepartment;
        if (!headOfDepartment)
          return <Badge variant={"outline"}>No added yet</Badge>;
        const { assumedOffice, endedOffice, position, user } = headOfDepartment;
        return (
          <div>
            <div>{user.name}</div>
            <div>
              {assumedOffice}-{endedOffice}
            </div>
          </div>
        );
      },
    },
    {
      ...
      header({ column }) {
        return (
          <DataTableColumnHeader column={column} title="Department Sectors" />
        );
      },
      cell({ row }) {
     
        const departmentalSectors = row.original.departmentalSectors;
           const numberOfDepartmentalSectors =
          departmentalSectors.length;
        if (numberOfDepartmentalSectors === 0) {
          return <Badge variant={"outline"}>Not added yet</Badge>;
        }
        if (numberOfDepartmentalSectors === 1) {
          return (
