						...
						<BreadcrumbItem>
							{isDesktop ? (
								<DropdownMenu open={open} onOpenChange={setOpen}>
									<DropdownMenuTrigger className="flex items-center gap-1" aria-label="Toggle menu" asChild>
									...
									<DropdownMenuContent align="start">
										{items.slice(1, -2).map((item, index) => (
											<DropdownMenuItem key={index} onClick={() => React.startTransition(() => {})}>
												<Link href={getNavigationLinkWithPathnameWithoutUpdate(item.href ? item.href : "#")}>
													{item.label}
												</Link>
											</DropdownMenuItem>
										))}
									...
									<DrawerContent>
										...
										<div className="grid gap-1 px-4">
											{items.slice(1, -2).map((item, index) => (
												<Link
													key={index}
													href={getNavigationLinkWithPathnameWithoutUpdate(item.href ? item.href : "#")}
					...
					<BreadcrumbItem key={index}>
						{item.href ? (
							<>
								...
								/>
								<BreadcrumbSeparator className={cn(index === itemsToDisplay - 2 && "hidden")} />
							</>
						) : (
							<ResponsiveNavigationMenuLink
...
function ResponsiveNavigationMenuLink({
	...
	return (
		<Comp asChild className={cn(isActive && "pointer-events-none font-bold text-foreground", className)}>
			<LoadingButton
				loading={isPending}
				onClick={() => startTransition(() => {})}
				variant={"ghost"}
