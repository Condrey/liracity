...
export function useUpsertDepartmentalSectorMutation() {
	...
	const mutation = useMutation({
		...
		onSuccess: async (data, variables) => {
			...
			const key2: QueryKey = ["sector", data.id];
			...
			queryClient.setQueryData<DepartmentData[]>(queryKey, (oldData) => {
				...
				if (!isSubmission) {
					return oldData.map((d) => ({
						...d,
						departmentalSectors: d.departmentalSectors.map((sector) => (sector.id === data.id ? data : sector))
					}));
				}

				return oldData.map((d) =>
					d.id === data.departMentId ? { ...d, departmentalSectors: [data, ...d.departmentalSectors] } : d
				);
			});
			queryClient.invalidateQueries({ queryKey: key2 });

			toast.success(data.departMent?.name, {
				description: `successfully ${isSubmission ? "Added" : "Updated"} ${data.name} sector to the ${
		...
		onError(error, variables, context) {
			console.error(error);
			toast.error(`Failed to ${variables.id ? "update" : "add"} this departmental sector.`);
		}
	});
	return mutation;
}
		...
		async onSuccess(data, variables, context) {
			...
			queryClient.setQueryData<DepartmentData[]>(
				...
				(oldData) =>
					...
					oldData.map((d) => ({
						...d,
						departmentalSectors: d.departmentalSectors.filter((ds) => ds.id !== data.id)
					}))
			);
			queryClient.invalidateQueries({ queryKey: key2 });
