...
function jt(e, t) {
	...
	if (((w += R), (I.precision = w), (r = Ae(O)), (n = r.charAt(0)), (o = Y(C)), Math.abs(o) < 15e14)) {
	...
	$o = fe(() => {
		...
		Me = (function () {
			...
			return function (n, i, o, s) {
				...
				for (f = n.e - i.e, Ee = K.length, Ke = ve.length, C = new Qr(Qu), O = C.d = [], w = 0; K[w] == (ve[w] || 0); )
					++w;
				if (
					(K[w] > (ve[w] || 0) && --f,
				...
				if (((ue = (ue / j + 2) | 0), (w = 0), Ee == 1))
					for (v = 0, K = K[0], ue++; (w < Ke || v) && ue--; w++)
						((V = v * ee + (ve[w] || 0)), (O[w] = (V / K) | 0), (v = (V % K) | 0));
				else {
					for (
						v = (ee / (K[0] + 1)) | 0,
				...
				return (O[0] || O.shift(), (C.e = f), $(C, s ? o + Y(C) + 1 : o));
			};
		})();
		ni = Fo(ic);
		ge = new ni(1);
		...
		return function (t, r) {
			...
			for (; a < o; )
				for (_ = r.charCodeAt(s + (w = a)), C = ++a, f = 0; f < Ke; f += 2)
					((O = ce[f]), (ce[f] = C = e(O, w, C, _, ce[f + 1])), (w = O));
			return C;
		};
	})();
});
...
k(Dr, "EngineVersionNotSupportedError");
u();
c();
p();
m();
...
async function Fd(e, t) {
	...
	if (o !== void 0 || n === "0.0.0" || n === "in-memory") {
		...
		try {
			...
			throw (console.error("JSON.parse error: body fetched from unpkg.com: ", R), O);
		}
		return C.version;
	}
...
`);
	...
	let R = { accelerate: f, ppg: w, driverAdapters: v };
	function C(O) {
		return O.length > 0;
	}
	return C(o)
		...
		constructor(n) {
			...
			try {
				...
				if (
					...
					for (let C of f.log) {
						let O = typeof C == "string" ? C : C.emit === "stdout" ? C.level : null;
						O &&
							this.$on(O, (I) => {
								Kt.log(`${Kt.tags[O] ?? ""}`, I.message || I.query);
							});
					}
			} catch (f) {
			...
			let i = n.middlewareArgsMapper ?? ef,
				...
				f = async (w) => {
					...
					let { runInTransaction: A, args: R, ...C } = w,
						O = { ...n, ...C };
					(R && (O.args = i.middlewareArgsToRequestArgs(R)),
						n.transaction !== void 0 && A === !1 && delete O.transaction);
					let I = await Aa(this, O);
					return O.model
						? Ea({
								result: I,
								modelName: O.model,
		...
		async _executeRequest({
			...
			try {
				...
				let O = { name: "serialize" },
					I = this._tracingHelper.runInChildSpan(O, () =>
						En({
							modelName: f,
							runtimeDataModel: this._runtimeDataModel,
							action: a,
